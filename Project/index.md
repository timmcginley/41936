...
```{note}
The existing auditorium can be converted to house the 200 capacity auditorium.
Multi use space can replace an existing auditorium if that multi use space can also be an auditorium.
In both cases it must not result in a total reduction of more than 20% of the existing capacity.
```
